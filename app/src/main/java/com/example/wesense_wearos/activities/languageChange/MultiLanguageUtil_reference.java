...
/*
 * 获取系统语言并做出语言更变的类，后续可作更改
 */
public class MultiLanguageUtil_reference {
    private static final String TAG = "MultiLanguageUtil_reference";
    private static volatile MultiLanguageUtil_reference instance;

    private MultiLanguageUtil_reference() {}

    public static MultiLanguageUtil_reference getInstance() {
        if (instance == null) {
            synchronized (MultiLanguageUtil_reference.class) {
                if (instance == null) {
                    instance = new MultiLanguageUtil_reference();
                }
            }
        }
        return instance;
